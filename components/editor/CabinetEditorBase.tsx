...
function getPeninWallAttachmentToWall(
  ...
  candidates.forEach((candidate) => {
    ...
    if (endpointDistance > tolerance || scalar < -tolerance || scalar > wallLength + tolerance) return;
    ...
    if (dragState.kind === "penin-wall") {
      ...
      if (
        detachedPanelWallIntersectsFloorCabinet(
          movedPeninWall,
          cabinets,
          allWalls.filter((candidateWall) => isThickWall(candidateWall) && !isDetachedPanelWall(candidateWall)),
          allWalls,
          dragState.id
        )
      ) {
